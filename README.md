# Shoreline Woodworks Website

## Figma Prototype

- This is the [Figma Prototype](https://www.figma.com/make/ztIH2APn6V2Js8JUqiKMhY/Carpentry-Business-Website?t=nti6zxyZi9QUghD4-20&fullscreen=1).
- You can generally base the front end work off of. It might change as we go.
- If you want to view the mobile prototype, open developer tools and toggle on the "Toggle Device Toolbar" (`shift` + `ctrl` + `m`).


## [Development Environment Setup](./docs/setup-dev-environment.md)
* [Prerequisites](./docs/setup-dev-environment.md#prerequisites)
* [Forking and Cloning](./docs/setup-dev-environment.md#forking-and-cloning)
* [Setting Up Your Dev Environment](./docs/setup-dev-environment.md#setting-up-your-dev-environment)

## [Managing Your Development Environment](./docs/managing-dev-environment.md)
* [Prisma](./docs/managing-dev-environment.md#prisma)  
* [Docker-PostgreSQL](./docs/managing-dev-environment.md#docker-postgresql)  
* [pnpm Commands](./docs/managing-dev-environment.md#pnpm-commands) 
* [Cloudinary](./docs/managing-dev-environment.md#cloudinary) 

## [Working with Forks in GitHub](./docs/git-workflow.md)
* [Creating a Branch](./docs/git-workflow.md#creating-a-branch)  
* [Committing and Pushing Your Work to GitHub](./docs/git-workflow.md#committing-and-pushing-your-work-to-github)
* [Creating a Pull Request (PR)](./docs/git-workflow.md#creating-a-pull-request)
* [Merging Upstream Back to Your Fork](./docs/git-workflow.md#merging-upstream-back-to-your-fork)
* [Notes](./docs/git-workflow.md#notes)

## [GitHub Notifications Setup](./docs/setup-gethub-notifications.md)
* [Update Watching Notifications](./docs/setup-gethub-notifications.md#update-watching-notifications)
* [Participating, @mentions and custom](./docs/setup-gethub-notifications.md#participating-mentions-and-custom)
* [Email Updates](./docs/setup-gethub-notifications.md#email-updates)

## [Testing](./docs/testing.md)
* [What is Cypress?](./docs/testing.md#what-is-cypress)
* [When to use Vitest v/s Cypress?](./docs/testing.md#when-to-use-vitest-vs-cypress)
* [Testing your changes](./docs/testing.md#testing-your-changes)